...
class PlayState extends MusicBeatState
{
	...
	override public function create()
	{
		...
		//FlxG.cameras.setDefaultDrawTarget(camGame, true); // lmao no this breaks the camera entirely
		...
		else if (SONG.song.toLowerCase() == 'startup' || SONG.song.toLowerCase() == 'bopeebo' /*this one is temp*/ || SONG.song.toLowerCase() == 'overwrite' || SONG.song.toLowerCase() == 'remnant') 
		{
			curStage = 'twitter';

			var bg:FlxSprite = new FlxSprite(-400, -10).loadGraphic(AssetPaths.backgroundish__png);
			bg.setSize(Std.int(bg.width * 1.25), Std.int(bg.height * 1.25));
			bg.updateHitbox();
			add(bg);
		}
		else
	...
	private function keyShit():Void
	{
		...
		{
			// Map direction index -> whether that key was just pressed
			var pressedKeys:Array<Bool> = [leftP, downP, upP, rightP]; // matches noteData 0,1,2,3

			// Collect all hittable player notes, sorted by how close they are to the strum time
			var possibleNotes:Array<Note> = [];
			notes.forEachAlive(function(daNote:Note)
			{
			...
			{
				return Std.int(Math.abs(a.strumTime - Conductor.songPosition) - Math.abs(b.strumTime - Conductor.songPosition));
			});
			...
			for (direction in 0...4)
			{
				...
				var laneNote:Note = null;
				for (daNote in possibleNotes)
				{
					if (daNote.noteData == direction)
					{
						laneNote = daNote;
						break; // already sorted by proximity, take the first match
					}
				}

				if (laneNote != null)
				{
					goodNoteHit(laneNote);
				}
				else
				{
					// Pressed a key with no hittable note in that lane = miss
					noteMiss(direction);
				}
			}

			// Sustain note holding (unchanged logic, just cleaned up)
			if (up || right || down || left)
			{
				notes.forEachAlive(function(daNote:Note)
				{
					if (daNote.mustPress && daNote.canBeHit && daNote.isSustainNote)
					{
						var holding:Bool = switch (daNote.noteData)
						{
							case 2: up;
							case 3: right;
							case 1: down;
						...
						if (holding && daNote.prevNote.wasGoodHit)
							goodNoteHit(daNote);
					}
				});
			}
		...
		// Idle animation when not holding
		if (boyfriend.holdTimer > Conductor.stepCrochet * 4 * 0.001 && !up && !down && !right && !left)
		{
			if (boyfriend.animation.curAnim.name.startsWith('sing') && !boyfriend.animation.curAnim.name.endsWith('miss'))
				boyfriend.playAnim('idle');
