...
class Character extends FlxSprite
{
	...
	public function new(x:Float, y:Float, ?character:String = "bf", ?isPlayer:Bool = false)
	{
		...
		switch (curCharacter)
		{
			...
			case 'user':
				// USER ANIMATION LOADING CODE
				tex = FlxAtlasFrames.fromSparrow(AssetPaths.user_ASSets__png, AssetPaths.user_ASSets__xml);
				trace("tex loaded: " + (tex != null));
				frames = tex;

				scale.set(1.5, 1.5);
				updateHitbox();
				animation.addByPrefix('idle', 'User idle dance', 24);
				animation.addByPrefix('singUP', 'User sing up', 24, false);
				animation.addByPrefix('singRIGHT', 'User sing right', 24, false);
				animation.addByPrefix('singDOWN', 'User sing down', 24, false);
				animation.addByPrefix('singLEFT', 'User sing left', 24, false);
				playAnim('idle');
