...
class Controls extends FlxActionSet
{
	...
	{
		super(name);
		FlxG.save.data.customBinds = FlxG.save.data.customBinds; // ensure this exists i think
		trace(FlxG.save.data.customBinds);
	...
	public function saveCustomBinds():Void
	{
		var savedBinds:Dynamic = {};

		for (control in Type.allEnums(Control))
		{
			Reflect.setField(savedBinds, Std.string(control), getInputsFor(control, Keys));
		}

		FlxG.save.data.customBinds = savedBinds;
		FlxG.save.flush();
		trace("Custom binds saved: " + Std.string(savedBinds));
	}

	public function loadCustomBinds():Void
	{
		if (FlxG.save.data.customBinds == null)
			return;

		var savedBinds:Dynamic = FlxG.save.data.customBinds;

		for (controlStr in Reflect.fields(savedBinds))
		{
			var control:Control = Type.createEnum(Control, controlStr);
			if (control == null)
			{
				trace('Skipping unknown field: $controlStr');
				continue;
			}

			var keys:Array<Int> = Reflect.field(savedBinds, controlStr);
			if (keys == null)
				continue;

			var flxKeys:Array<FlxKey> = keys.map(function(k) return cast k);
			var existingKeys:Array<FlxKey> = getInputsFor(control, Keys).map(function(k) return cast k);

			unbindKeys(control, existingKeys);
			bindKeys(control, flxKeys);

			trace('Loaded bind for $controlStr -> $keys');
		}
	}
